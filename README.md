...
1. Download `pre-commit` and `pre-commit.ps1` from this repo
2. Copy both to `.git/hooks/` in your repository
3. Ensure `pre-commit` has execute permissions
...
```
.exe .dll .png .jpg .jpeg .gif .ico .woff .woff2 .ttf .eot .pdf .zip .7z .rar
```
...
Yes! Configure a global hooks directory:

```powershell
# Create global hooks directory
$globalHooks = "$env:USERPROFILE\.git-hooks"
New-Item -ItemType Directory -Path $globalHooks -Force

# Copy hooks there, then configure git
